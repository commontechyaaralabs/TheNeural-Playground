#!/usr/bin/env python3
"""
Test Teacher-Classroom-Student-Demo Project System
This script demonstrates the complete workflow:
1. Teacher registration
2. Classroom creation
3. Demo project creation
4. Student joining with demo access
...
class TeacherSystemTester:
    """Test class for the teacher-classroom-student-demo project system"""
    ...
    async def test_teacher_registration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            teacher_data = TeacherCreate(
                name="John Smith",
                school_name="Tech Academy High School",
            ...
            print(f"   Training Date: {teacher.date_of_training}")
            print(f"   Session: {teacher.session}")
    ...
    async def test_student_join(self, hashcode: str, student_name: str) -> Dict[str, Any]:
        """Test student joining classroom using hashcode"""
        print(f"\nTesting Student Join: {student_name}...")
        print("=" * 50)
        
        try:
            # Create student join data
            ...
            print(f"   Hashcode Used: {student.hashcode}")
    ...
    async def test_multiple_students(self, hashcode: str, classroom_suffix: str = ""):
        """Test multiple students joining the same classroom"""
        print(f"\nTesting Multiple Students Join...")
        print("=" * 50)
        
        base_names = ["Alice Johnson", "Bob Lee", "Charlie Brown"]
        student_names = [f"{name} {classroom_suffix}".strip() for name in base_names]
        
        for name in student_names:
            try:
                await self.test_student_join(hashcode, name)
            except Exception as e:
                print(f"Student {name} join failed: {str(e)}")
                continue
    
    async def test_demo_project_access(self, student_id: str):
        """Test that student can access demo projects"""
        print(f"\nTesting Demo Project Access...")
        print("=" * 50)
        
        try:
            # Get student's accessible demos
            accessible_demos = await self.student_service.get_student_demos(student_id)
            
            print(f"Student can access {len(accessible_demos)} demo projects:")
            for demo in accessible_demos:
                print(f"   - {demo['name']} ({demo['project_type']})")
                print(f"     Description: {demo['description']}")
            
            return accessible_demos
            
        except Exception as e:
            print(f"Demo project access failed: {str(e)}")
            raise
    ...
    async def run_complete_test(self):
        ...
        print("Starting Complete Teacher-Classroom-Student-Demo Project System Test")
        print("=" * 60)
        ...
        try:
            ...
            await self.test_multiple_students(first_hashcode, "(Class 1)")
            ...
            await self.test_multiple_students(second_hashcode, "(Class 2)")
            
            # 6. Test demo project access for students (test first classroom student who has access to demos)
            first_classroom_students = [key for key in self.test_results.keys() if key.startswith('student_') and '(class_1)' in key.lower()]
            if first_classroom_students:
                # Get the first student from the first classroom (should have demo access)
                first_student_key = first_classroom_students[0]
                await self.test_demo_project_access(self.test_results[first_student_key].student_id)
            
            # 7. Test classroom demos
            ...
            # 9. Test project linking for one student (use second classroom student)
            second_classroom_students = [key for key in self.test_results.keys() if key.startswith('student_') and '(class_2)' in key.lower()]
            if second_classroom_students:
                await self.test_project_linking(self.test_results[second_classroom_students[0]].student_id)
